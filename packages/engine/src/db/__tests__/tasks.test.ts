...
describe('db/tasks', () => {
  ...
  describe('getTasksByState', () => {
    it('passes correct filter params', async () => {
      ...
      // getTasksByState chain: select → eq → eq → is
      const mockIs = vi.fn().mockResolvedValue({ data: [sampleTaskRow], error: null });
      const outerChain = {
  ...
  describe('getDependentTasks', () => {
    it('filters by depends_on array containment', async () => {
      ...
      const mockIs = vi.fn().mockResolvedValue({ data: [dependentRow], error: null });
      const mockContains = vi.fn().mockReturnValue({ is: mockIs });
