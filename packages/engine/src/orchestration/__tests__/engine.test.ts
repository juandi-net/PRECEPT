...
// Mock transitive dependencies that require env vars
...
describe('OrchestrationEngine', () => {
  ...
  it('processes pushed events', async () => {
    engine.push({ type: 'weekly_planning', orgId: 'org-1' });
    ...
    expect(handledEvents[0]).toEqual({ type: 'weekly_planning', orgId: 'org-1' });
  });

  it('processes events sequentially', async () => {
    const order: number[] = [];
    vi.spyOn(engine, 'handleEvent').mockImplementation(async (event) => {
      const idx = event.type === 'weekly_planning' ? 1 : 2;
      order.push(idx);
  ...
  it('routes task_terminal event to handleEvent', async () => {
    engine.push({ type: 'task_terminal', orgId: 'org-1', taskId: 'task-1' });

    await vi.waitFor(() => {
      expect(handledEvents).toHaveLength(1);
    });

    expect(handledEvents[0]).toEqual({ type: 'task_terminal', orgId: 'org-1', taskId: 'task-1' });
  });
