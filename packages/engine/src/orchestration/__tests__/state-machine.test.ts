...
describe('validateTransition', () => {
  ...
  const ownerOverrideTransitions: [TaskState, TaskState][] = [
    ['REVIEW', 'IN_PROGRESS'],
    ['POLISH', 'IN_PROGRESS'],
  ...
  it.each(ownerOverrideTransitions)('%s → %s is valid (owner override)', (from, to) => {
    expect(validateTransition(from, to)).toBe(true);
  });
...
describe('applyTransition', () => {
  ...
  it('owner override: REVIEW → IN_PROGRESS via applyTransition', async () => {
    vi.mocked(getTask).mockResolvedValue({
      id: 'task-1', org_id: 'org-1', state: 'REVIEW',
      revision_count: 0,
      polish_count: 0,
    } as any);

    const result = await applyTransition('task-1', 'IN_PROGRESS', 'Owner', 'owner feedback override');

    expect(updateTaskState).toHaveBeenCalledWith('task-1', 'IN_PROGRESS');
    expect(logTransition).toHaveBeenCalledWith({
      orgId: 'org-1',
      taskId: 'task-1',
      fromState: 'REVIEW',
      toState: 'IN_PROGRESS',
      agentId: 'Owner',
      reason: 'owner feedback override',
    });
    expect(result).toBe('IN_PROGRESS');
  });
});

describe('state-machine: escalation resolution transitions', () => {
  it('allows ESCALATED → FAILED', () => {
    expect(validateTransition('ESCALATED', 'FAILED')).toBe(true);
  });

  it('allows ESCALATED → ACCEPTED', () => {
    expect(validateTransition('ESCALATED', 'ACCEPTED')).toBe(true);
  });

  it('still allows existing ESCALATED transitions', () => {
    expect(validateTransition('ESCALATED', 'QUEUED')).toBe(true);
    expect(validateTransition('ESCALATED', 'PLANNED')).toBe(true);
    expect(validateTransition('ESCALATED', 'IN_PROGRESS')).toBe(true);
  });
});
