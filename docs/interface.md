...
See `structure.md` for the organizational hierarchy, `orchestration.md` for the engine, `memory.md` for how the system remembers, `skills.md` for procedural memory, `onboarding.md` for the first interface experience, `security.md` for data classification and access controls, `techstack.md` for the infrastructure that implements this interface, `passion.md` for the philosophical foundation behind why this interface must honor the owner's passion.
...
**Why sign-offs exist (from passion.md):** Spectatorship is the fastest way to extinguish an owner's passion. An organization that runs without the owner's hands ever touching anything slowly disconnects the owner from their own story. The sign-off is the structural countermeasure — not because the owner's hands are technically necessary, but because the act of closing the loop is the act of saying "I am building this." The system cannot do that for them. See `passion.md` — Part III, Enemy #1.
