...
precept/
  packages/
    ...
    shared/     — Types, Cornerstone schema, constants
```
...
| Validation | Zod | — | Runtime schema validation for AI responses. Every `invokeAgent` result is validated against a typed schema with per-role failure policies. |
...
| Role | Model | Purpose |
|---|---|---|
| CEO | Opus 4.6 | Cornerstone session, strategic planning, initiative decomposition |
...
│ • Cornerstone editor│            │   → Tunnel → Engine │
└────────┬────────────┘            └──────────┬──────────┘
...
3. **cloudflared** — Cloudflare Tunnel. Exposes port 3001 at `api.precept.so` for Resend webhooks and Vercel frontend API calls.
...
Cloudflare Tunnel exposes the engine's webhook endpoint (for Resend inbound emails) via a public URL (`api.precept.so`). The tunnel runs as a macOS launchd service alongside the engine and CLIProxy.
...
### Why Resend
Each PRECEPT org needs branded email — `ceo@mail.example.com`, `ceo@mail.example.org` — not a generic platform address. Resend's free tier includes 1 custom domain, inbound email via webhooks, and 100 emails/day. In the multi-org model, each org owner creates their own free Resend account with their own domain, so PRECEPT scales to unlimited orgs at zero email cost. Reply parsing is handled by the CEO via LLM-based intent extraction rather than a built-in email-to-JSON service.
