...
See `orchestration.md` for the engine that implements this structure, `memory.md` for the knowledge architecture, `skills.md` for procedural memory and the Curator role, `security.md` for data classification and access controls, `interface.md` for how the owner interacts with this organization, `passion.md` for why passion is the precondition for everything working.
...
| Root before strategy | passion.md | CEO reads the owner's root before every planning cycle. Metrics serve the mission, not the reverse. |
| Leadership as propagation | Isaiah 28:10, this system | The system's purpose is propagating the owner's values at scale with mechanical fidelity. Every design decision is evaluated by: does this improve value-propagation fidelity, or introduce drift? |
...
BOARD (Owner)
  ...
  └── CEO (Opus — weekly planning + event-driven)
        ...
        The CEO is also the steward of the owner's root. If the
        organization's work drifts from the founding passion captured in
        the Cornerstone root field, that is a structural failure —
        not just a strategic one. Before every planning cycle, the CEO
        reads the root field first.
        │
...
| **Root Stewardship** | CEO reads root field before every planning cycle. Plans are evaluated against the root, not just the metrics. Drift from root is surfaced to the owner as a Sign-Off, not silently corrected. |
