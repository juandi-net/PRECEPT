...
See `structure.md` for the organizational hierarchy and evaluation flow, `skills.md` for procedural memory and the Curator role, `interface.md` for the design philosophy behind briefings, reply parsing, and The Interface, `passion.md` for why the system must serve the owner's founding passion. This document covers the technical orchestration that implements that structure.

## Engine Architecture

The engine is a standalone TypeScript service running locally on the Mac Mini as a macOS launchd service. It is stateless — all execution state lives in Supabase. The engine can restart at any time without data loss. Resend webhooks and Vercel frontend API calls reach the engine via Cloudflare Tunnel (`api.precept.so` → `localhost:3001`).
...
4. **Onboarding completion** — `/api/onboarding/complete` endpoint triggers the first CEO cycle after Lock & Launch (see `onboarding.md`). The Scribe is skipped for this first invocation — no activity to compress yet. CEO receives the Cornerstone only.

### Handler Modules

The orchestration core is split into focused handler modules, each responsible for a domain of the task lifecycle:

| Module | Responsibility |
|--------|---------------|
| `handlers/planning.ts` | Planning cycles (monthly/weekly/daily/adhoc), briefings, plan approval |
| `handlers/execution.ts` | Worker dispatch, background execution, task completion, dependency-based rolling dispatch |
| `handlers/evaluation.ts` | Review verdicts, judge verdicts, escalation diagnosis |
| `handlers/owner.ts` | Owner email reply processing, direct task feedback |
| `handlers/maintenance.ts` | Recovery from restart, curator skill extraction, phase completion |

The engine itself (`engine.ts`) is a thin dispatcher: it owns the event queue, routes events to the correct handler, and exposes cross-cutting utilities (worker execution, ready-task dispatch) via the `EngineContext` interface.
...
The owner can force any task back to `IN_PROGRESS` via direct feedback, regardless of current state. This is modeled as a first-class state machine transition (not a bypass), so all owner overrides appear in the audit log via `applyTransition`. The `ACCEPTED` → `IN_PROGRESS` path exists solely for owner override — accepted tasks can be reopened if the owner provides corrective feedback.
...
The Dispatcher does NOT receive the Cornerstone, strategic rationale, or evaluation details. It manages logistics, not strategy.
...
Scribe compresses into:
  ...
  • Pattern observations ("Writer revised 3x this week on value props — possible Cornerstone gap")
  • Skill changes (new/refined skills from Curator since last cycle)
  • Forward context (upcoming deadlines, resource status)
  │
  ▼
CEO context package:
  Cornerstone (full, always present)
...
|---|---|---|
| **CEO** | Cornerstone + Scribe's compressed output + lessons + owner input + skill changes | Supabase → Scribe → CEO |
| **Board Advisor** | CEO's proposed plan + Cornerstone + recent decision log + performance data + lesson artifacts | Supabase direct |
...
**Root-first planning:** Before generating initiatives, the CEO reads the `root` field of the Cornerstone. Every initiative is evaluated against the question: does this serve the root, or just the metrics? If the CEO identifies drift — the organization is executing well but moving away from the owner's founding passion — it surfaces this as a Sign-Off rather than silently correcting course. The owner decides whether the root has evolved or the plan has drifted.
...
3. **Engine recovery scan** — single query fetches all tasks in recoverable states (`QUEUED`, `DISPATCHED`, `IN_PROGRESS`, `REVIEW`, `POLISH`, `REVISION`, `JUDGMENT`) via `getTasksByStates()`, then groups by state in memory for processing:
   - Tasks in QUEUED → re-dispatch and execute
   - Tasks in DISPATCHED/IN_PROGRESS → check against configurable timeout (`TASK_TIMEOUT_MS`, default 600s)
     - Timed out → mark FAILED
     - Recent → leave in place (worker may still be running)
   - Tasks in REVIEW/JUDGMENT → re-push evaluation events to the queue
   - Tasks in POLISH/REVISION → transition to REVIEW, re-invoke reviewer
...
Engine Process
  ...
  └── Orchestration core (event-driven)
        ├── Event queue processed sequentially (no concurrent CEO/Dispatcher calls)
        │   This prevents contradictory decisions
        │
        └── Worker execution is fire-and-forget via Semaphore
            Bounded by WORKER_CONCURRENCY_LIMIT (env, default 3)
            Workers push task_completed events back to queue when done
```

The event queue (`drain()`) processes events sequentially — if the CEO and Dispatcher could run simultaneously, they might make contradictory decisions. Worker *execution* is the exception: `handlePlanApproved`, `dispatchReadyTasks`, and `handleDispatchTask` use `executeWorkerInBackground()` to fire-and-forget worker runs bounded by a counting `Semaphore`. Workers push `task_completed` events back to the queue when done, so evaluation (review/judge) stays sequential.

This means the event queue stays responsive during plan execution — briefings, owner replies, and escalations process while workers run, rather than blocking for 30+ minutes behind sequential worker execution.

Worker *rework* paths (review polish, judge revision, owner feedback) remain synchronous because the caller needs the result immediately to decide the next state transition.

All AI calls — including onboarding, workers, reviewers, judges, advisors, and the curator — route through `invokeAgent()`, which provides retry with exponential backoff, structured JSON extraction, and automatic audit logging.
